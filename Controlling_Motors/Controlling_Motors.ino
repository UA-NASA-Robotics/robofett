...
void billieJeanEmote() {
  if(isEmoting) return;
  isEmoting = true;

  Serial.println("emoting");
...
void playSong(Note* notes, int tempo) {
  ...
  noTone(BUZZER_PIN);
  for(int i = 0; i < 27; i++) {
    int duration = 1000 * notes[i].duration / (tempo / 60);
    if(notes[i].frequency <= 0) {
      noTone(BUZZER_PIN);
      delay(notes[i].duration);
    } else {
      tone(BUZZER_PIN, notes[i].frequency);
      delay(duration * 2/3);
      noTone(BUZZER_PIN);
      delay(duration * 1/3);
    }
